...
We design our experiments to answer two questions, each tied to a component of our contribution:

% \ali{for each following question, we should specify the sections that address them.}  \varun{+1}
%\varun{if we have a squishenumerate instead of enumerate, use that?}
...
\begin{enumerate}
    ...
    % \item[\textbf{Q3.}] Do fine-tuning methods and inference-time methods occupy systematically different regions of the trade-off frontier, consistent with the mechanistic analysis in~\cref{app:method_analysis}?
\end{enumerate}


\subsection{Experimental Setup}
\label{subsec:setup}
\paragraph{Base model and target concepts.}
...
four high-$\hat\kappa$ animal-subtype concepts (\texttt{horse}, \texttt{dog}, \texttt{cat}, \texttt{bear}, $\hat\kappa \in [0.77, 0.89]$) and one low-$\hat\kappa$ architectural concept (\texttt{castle}, $\hat\kappa = 0.27$). Each target is evaluated against its discovered 10-concept neighborhood and a 3-concept control set (\cref{tab:kappa}); per-target prompt sets 
are detailed in~\cref{app:eval_pipeline}.
% \begin{enumerate}
...
We measure seven metrics spanning the two axes of~\cref{thm:tradeoff}, all metrics are formally defined in~\cref{app:metric_defs}. \emph{Erasure metrics} quantify target suppression: Unlearning Accuracy (UA), Indirect Recovery Rate (IRR), and Indirect Retention Accuracy (IRA). \emph{Retention metrics} quantify collateral damage: Neighbor Preservation (NP), Control Preservation (CP), and DamageGap ($= \mathrm{CP} - \mathrm{NP}$). 
All metrics are reported both absolutely and as relative changes from  $\mathcal{D}_\theta$ to enable cross-concept comparison.
...
\paragraph{No method escapes the trade-off.}
\Cref{tab:main_results_avg} reveals a consistent pattern across the seven methods: no method achieves simultaneously high erasure strength (high UA, low IRR) and high retention (high IRA, high NP). Robust fine-tuning methods push UA close to $99$ but reduce NP to $18$--$76$; inference-time methods preserve neighbors at NP $\geq 59$ but leave residual erasure gaps reflected in IRR above $24$. Standard fine-tuning methods fall between these extremes. 
...
Operating points trade off $\varepsilon$-robustness for $\gamma$-neighbor preservation along a frontier whose shape is determined by $\kappa$ and $L$ as predicted by Eq.~\eqref{eq:tradeoff_eps}.

\paragraph{The trade-off is structural, not a measurement artifact.}
...
\Cref{thm:tradeoff} predicts that trade-off severity scales with the entanglement coefficient $\kappa$: target concepts whose activation regions overlap substantially with neighbors should suffer greater collateral damage under robust erasure. We test this across five target concepts with estimated $\hat\kappa$ span from $0.27$ (\texttt{castle}) to $0.89$ (\texttt{dog}); see~\cref{tab:kappa} and~\cref{app:kappa_estimation} for the estimation procedure. 

\paragraph{$\hat\kappa$ predicts collateral damage on neighbors.}
...
\paragraph{The pattern generalizes beyond a single method.}
...
\end{figure}
\paragraph{$\hat\kappa$ measures activation-space overlap, not semantic similarity.}
A natural concern is whether $\hat\kappa$ proxies for some other concept-level property such as semantic category (animal vs.\ architecture). Our estimator (\Cref{app:kappa_estimation}) is computed from sample-level overlap of activation regions, not from category labels or human-judged similarity. \texttt{Castle} ($\hat\kappa = 0.27$) and the four animal concepts ($\hat\kappa \in [0.77, 0.89]$) differ in $\hat\kappa$ not because they belong to different taxonomic categories but because castle's discovered neighbors (\texttt{fortress}, \texttt{palace}, \texttt{citadel}) occupy distinct sub-regions of activation space despite being semantically related, while animal subtypes (e.g., \texttt{puppy}, \texttt{labrador} for \texttt{dog}; \texttt{mare}, \texttt{pony} for \texttt{horse}) densely overlap their target's region. 
...
\paragraph{\Cref{thm:tradeoff} is a lower bound, not a ceiling.}
...
% \paragraph{Q3: Mechanism type determines the operating region.}
